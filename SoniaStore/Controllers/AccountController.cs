    ...
    public async Task<IActionResult> Login(LoginVM login)
    {
        if (ModelState.IsValid)
        {
            ...
            if (result.IsLockedOut)
            {
                ...
                ModelState.AddModelError(string.Empty, "Sua conta não está confirmad, verifique seu email!!");
            }
            else 
    ...
    public async Task<IActionResult> Registro(RegistroVM registro)
    {
        if (ModelState.IsValid)
        {
            ...
            foreach (var error in result.Errors) 
                ModelState.AddModelError(string.Empty, TranslateIdentityErrors.TranslateErrorMenssage(error.Code));
        }
